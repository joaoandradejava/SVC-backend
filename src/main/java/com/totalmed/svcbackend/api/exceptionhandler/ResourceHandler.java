...
public class ResourceHandler extends ResponseEntityExceptionHandler {
	private static final String STANDARD_MESSAGE = "Ocorreu um erro inesperado, se o problema persistir recomendo que entre em contato com o desenvolvedor da API.";
	...
	public ResponseEntity<Object> handleAccessDenied(AccessDeniedException ex, WebRequest request) {
		...
		String message = "Acesso negado. Você não tem permissão para acessar essa funcionalidade do sistema!";
		ProblemDetail problemDetail = new ProblemDetail(error.getType(), error.getTitle(), status.value(), message,
				message);
	...
	public ResponseEntity<Object> handleEnvioEmail(EnvioEmailException ex, WebRequest request) {
		HttpStatus status = HttpStatus.INTERNAL_SERVER_ERROR;
		Error error = Error.ERRO_INTERNO_NO_SERVIDOR;
		String message = ex.getMessage();
		ProblemDetail problemDetail = new ProblemDetail(error.getType(), error.getTitle(), status.value(), message,
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		Error error = Error.ERRO_NA_DESSERIALIZACAO_DO_JSON;
		String message = "Ocorreu um erro na desserializacao do JSON, verifique se a sintaxe do JSON foi inserida corretamente.";
		ProblemDetail problemDetail = new ProblemDetail(error.getType(), error.getTitle(), status.value(), message,
				STANDARD_MESSAGE);
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		String message = "Ocorreu um erro de validação no corpo da requisição. Verifique se todos os campos foram inseridos corretamente no corpo da requisição.";
		ProblemDetail problemDetail = new ProblemDetail(error.getType(), error.getTitle(), status.value(), message,
				message);
	...
	protected ResponseEntity<Object> handleHttpRequestMethodNotSupported(HttpRequestMethodNotSupportedException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		Error error = Error.METODO_DE_REQUISICAO_NAO_SUPORTADO;
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
		...
		String message = String.format("Nenhum endpoint encontrado para '%s%s'", ex.getHttpMethod(),
				ex.getRequestURL());
		ProblemDetail problemDetail = new ProblemDetail(error.getType(), error.getTitle(), status.value(), message,
				STANDARD_MESSAGE);
	...
	protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
		...
		String message = String.format(
				"Ocorreu uma falha ao tentar converter o tipo %s com o valor '%s' para o tipo necessario que é o tipo %s",
